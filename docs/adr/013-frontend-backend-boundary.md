...
- Sensor I/O over Bluetooth (Racelogic Mini, OBDLink MX, Pixel Earbuds pairing) — the radios are physically on the device.
...
- Persistence (DuckDB sessions / laps / coaching_notes; future telemetry frames).
- Antigravity store-and-forward Tx (queue management, retry).
...
| `pipeline/AntigravityPipeline.kt` | **Keep** for now — Tx-side burst assembly happens on-device next to the sensors. Long-term: the bridge owns this too once Termux deployment matures. |
...
|---|---|---|
| `POST /fuse` | `SensorFusion.kt` | Accept raw Racelogic + OBDLink frames; return confidence-annotated `TelemetryFrame` |
