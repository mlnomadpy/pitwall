...
# src/simulator is on the bridge's sys.path — replicate that here so tools can
# import sonoma, gold_standard, etc. without duplicating the path hack.
_SIM_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "src", "simulator"))
if _SIM_DIR not in sys.path:
    sys.path.insert(0, _SIM_DIR)

_GOLD_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "data", "reference", "sonoma_gold.json")
)
...
def get_audio_script_context(corner_name: str, driver_level: str = "intermediate") -> dict[str, Any]:
    ...
    try:
        import sonoma as _sonoma
        from coach_engine import match_bentley_concept
    except ImportError as e:
